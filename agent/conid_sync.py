...
def normalize_entry(key: str, d: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if out["secType"] == "CASH" and out.get("symbol") and len(out["symbol"]) >= 6:
        ...
        if s[:3].isalpha() and s[-3:].isalpha():
            out["symbol"] = s[:3]; out["currency"] = s[-3:]
        out["exchange"] = "IDEALPRO"
    if out["secType"] == "CMDTY" and out.get("symbol") and len(out["symbol"]) >= 6:
        s = out["symbol"].upper(); base, quote = s[:3], s[-3:]
        if base in {"XAU","XAG","XPT","XPD"} and quote.isalpha():
            out["symbol"] = base; out["currency"] = quote
    return out

def to_contract(key: str, d: Dict[str, Any]) -> Contract:
    d = normalize_entry(key, d)
    c = Contract()
    if d.get("conId"): c.conId = int(d["conId"])
    c.secType  = d.get("secType"); c.symbol = d.get("symbol")
    c.exchange = d.get("exchange"); c.currency = d.get("currency")
...
class _Resolver(EWrapper, EClient):
    ...
    def _get_req_id(self) -> int:
        if not self._next_ready.wait(10):
            raise RuntimeError("Brak nextValidId z TWS/Gateway")
        with self._lock:
...
def ensure_conids(path="config/symbols.yaml", host="127.0.0.1", port=4003, client_id=123, save_in_place=True) -> bool:
    fp = Path(path)
